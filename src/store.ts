...
const initialState: AppWalkState = localStorage.getItem('reduxState') ? JSON.parse(localStorage.getItem('reduxState')!).counter : {
    step: {
        value: 0,
...
function handleChange() {
    ...
    if (activeTurnValue != previousValue && activeTurnValue === 1) {
        setTimeout(() => { store.dispatch(battleSlice.actions.p2Attack({ moveId: 1 })) }, 1000)
    }
}
...
const battleSlice = createSlice({
    ...
    reducers: {
        p1Attack: (state, { payload: { moveId } }: PayloadAction<Attack>) => {
            ...
            state.battleLog.unshift(`${denjuuList[state.p1.denjuuId].displayId} used ${moveList[moveId].displayId}`)
        },
        p2Attack: (state, { payload: { moveId } }: PayloadAction<Attack>) => {
            ...
            state.battleLog.unshift(`${denjuuList[state.p2.denjuuId].displayId} used ${moveList[moveId].displayId}`)
...
export const store = configureStore({
    reducer: combineReducers({ counter: counterSlice.reducer, battle: battleSlice.reducer, application: applicationSlice.reducer, events: eventSlice.reducer })
});
